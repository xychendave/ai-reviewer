...
def get_settlement(franchisee, start_date, end_date):
    ...
    df_with_shop_count_and_scene["单个客服预计结算金额"] = round(df_with_shop_count_and_scene["机器人接待人次"] * 0.134, 2)
    ...
    df_merge["坐席基数"] = df_merge["总在线时长"].apply(lambda x: max(0, x - (30 * 2)) // (30 * 8) + 1)
    df_merge["单个客服实际结算金额"] = df_merge["单个客服预计结算金额"].apply(
        lambda x: 180 if x > 180 else 0)
    df_merge["坐席基数"] = df_merge.apply(lambda x: 0 if x["单个客服实际结算金额"] == 0 else x["坐席基数"], axis=1)
    df_merge["单个客服实际结算金额"] = df_merge.apply(lambda x: x["单个客服实际结算金额"] * x["坐席基数"], axis=1)
    ...
    total_sum = [{
        ...
        "结算方式": "按坐席数量",
        "坐席单价": 180,
        "坐席数量": df_merge_new["坐席基数"].sum(),
        "实际结算金额": df_merge_new["单个客服实际结算金额"].sum()
    }]
...
def settlement_tab():
    ...
    with gr.Tab("加盟商金额结算"):
        ...
        button.click(
            get_settlement,
            inputs=[franchisee_input, start_date, end_date],
